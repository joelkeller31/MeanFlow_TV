...
if __name__ == "__main__":
    ...
    # p_init = GaussianGenerator(n_dims=2, noise_std=0.5)
    p_target = CrescentGenerator(R=1.0, r=0.75, d=0.6)
    ...
    model_original = trainer_backward.train_mean_flow_model( num_batches_per_epoch, n_epochs, lr=5e-4)
    
    model_combined = trainer_combined.train_mean_flow_model(num_batches_per_epoch, n_epochs, lr=5e-4)

    model_dict = { 'Original': model_original, 'Combined': model_combined }
    plot_and_save_trajectories(model_dict, n_samples=2048, n_steps=10, save_dir="trajectory_plots", p_init=p_init, p_target=p_target)
